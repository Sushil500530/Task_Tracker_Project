...
const EditTask = ({ editTask, setIsOpen, refetch }) => {
    ...
    const handleCreateTask = async (data) => {
        ...
        try {
            await getLink.put(`/updated-task/${editTask?._id}`, taskData)
                .then(res => {
                    refetch();
        ...
        <div>
            ...
            <form onSubmit={handleSubmit(handleCreateTask)} className="space-y-5">
                <div className="flex items-center justify-between">
                    ...
                    <input type="text" name="title" readOnly defaultValue={editTask?.title} className="border w-full bg-[#c5c0c0] p-2 cursor-not-allowed" {...register("title", { required: true })} required />
                </div>
                <div className="flex items-center justify-between">
                    <label className="w-1/3 text-base font-bold">Description:</label>
                    <textarea name="description" id="" readOnly defaultValue={editTask?.description} className="border w-full bg-[#c5c0c0] p-2 cursor-not-allowed" cols="10" rows="3"  {...register("description", { required: true })} required></textarea>
                </div>
                <div className="flex items-center justify-between">
                    <label className="w-1/3 text-base font-bold">Team:</label>
                    <input type="text" name="team" readOnly defaultValue={editTask?.team} className="border w-full bg-[#c5c0c0] p-2 cursor-not-allowed"  {...register("team", { required: true })} required />
                </div>
                <div className="flex items-center justify-between">
                    <label className="w-1/3 text-base font-bold">Assignees:</label>
                    <input type="text" name="assignees" readOnly defaultValue={editTask?.assignees} className="border w-full bg-[#c5c0c0] p-2 cursor-not-allowed"  {...register("assignees", { required: true })} required />
                </div>
                <div className="flex items-center justify-between flex-col md:flex-row lg:flex-row gap-3">
                    <div className="flex items-center justify-start gap-5">
                        ...
                        <select name="priority" id="sortBy" className="p-2 py-1 rounded bg-[#c5c0c0]" {...register("priority", { required: true })} required>
                            <option disabled selected defaultValue={editTask?.priority} >{editTask?.priority}</option>
                    ...
                    <div className="flex items-center justify-start gap-5">
                        ...
                        <select name="status" className="px-2 py-1 rounded bg-[#c5c0c0] text-black p-2" {...register("status", { required: true })} required>
                            <option disabled selected defaultValue={editTask?.status}>{editTask?.status}</option>
